# 職務経歴書

## 基本情報

key       | value
--------- | ----------------------------------------------------
Name      | 横山 翔大 (Syoudai Yokoyama)
Twitter   | [@\_quantumman\_](https://twitter.com/_quantumman_)
Instagram | [@quantumman](https://www.instagram.com/quantumman/)
LinkedIn  | https://www.linkedin.com/in/syoudai-yokoyama-80167927
GitHub    | https://github.com/quantumman

## スキル

### 言語
#### プログラミング言語
##### C# 5 ~ 7
- 業務で主に使用しています。業務をする上で必要な読み書きは問題なく出来ます。
- 最新の言語仕様(C# 7)も問題なく使えます。
##### Haskell
- 趣味で学習してました。
- 業務を使える程度の読み書きは問題はありません。
- Webフレームワーク使って簡単なWebアプリ作れる程度です。
##### Elm
- 個人プロジェクトのフロントエンド(SPA)に採用していました。
- ルーティング、JSONの扱い、JavaScriptコードの呼び出し、Elm Architecture などフロントエンドに必要なことは一通り経験しています。
##### JavaScript (ES5, ES6, ES7)
- 業務、趣味で触っています。
- 業務ではjQueryからReact/Vue.jsまでを触っています。
- 最近はbabelを使ってES7で書くことが多いですが、業務によってはES5で書いています。
##### TypeScript 1.7
- 業務でReactと一緒に使いました。
- 業務で必要な読み書き以外に、型定義もある程度自前で書いてました。
##### HTML, CSS
- Webアプリケーションをやる上で必須なので使えます。
- CSSはベンダープレフィックスを避けるため、PostCSSでautoprefixerを使うことが多いのでベンダープレフィックスについては詳しくないです。
##### Kotlin
- Android案件をやっていた時にJavaを捨てたいが為に個人的に勉強していました。
- 正直、今はあんまり構文覚えてないので使うならリハビリが必要です。
- Emacs(エディタ)のKotlinモード作りました。今はメンテナは別の人になってます。
##### Elixir
- 個人プロジェクトでPhoenix Framework を使って、Webアプリケーション使ってました。(ちなみにフロントエンドはElm)
- 正直、今はあんまり構文覚えてないので使うならリハビリが必要です。

##### 日本語
- ネイティブ
##### 英語
- 技術文書読める
- githubでPR出せるぐらい
- 日常会話はブロークンイングリッシュ程度でなら何とかできます

### フレームワーク
- ASP.NET Web Forms ベースの社内フレームワーク
- ASP.NET MVC/Web API
- ASP.NET Core 1.2
- .NET Framework (3.5/4.0/4.5.2/4.7)
- EntityFramework 4/5
  - .NET の代表的な ORM
- jQuery
  - 1.x ~ 2.x
- React
- AngularJS
  - 1.x
- Dynamics CRM
  - F/W というより Microsoft 謹製のカスタマイズ性の高いCRM
- Bootstrap
  - CSS framework
- Semantic UI
  - CSS frameowrk
- Vue.js (2.x)
- Vuetify (0.7.x)
  - Vue.js のMaterial Design UIフレームワーク
  - 私の推しUIフレームワーク

#### 個人で触ったことのあるもの
- Phoenix Framework
  - Elixir の fullstack web framework
- Yesod
  - Haskell の fullstack web framework
- Mithril.js

### その他
- 言語やフレームワークに限らないスキル。開発手法やプロセス、ツールなど
  - Git
  - TDD/BDD
  - Nodejs
    - gulp, webpack etc..
    - 個人プロジェクトのフロント開発でよく使う
  - Issue tracker
    - Redmine
    - Backlog
  - Cloud
    - Azure
    - Heroku (ちょっとだけ)
  - JSON (Hyper) Schema
- 好きなエディタ
  - Emacs
  - Visual Studio Code

## 強み
- 新しいことが好きなのでエッジの効いた技術を楽しく学べる

## 弱み
- 慢性的な長時間労働が強制されると心が折れる
- Micrsoft製品を使うと心が折れる

## やったことはないが興味があるもの

- iOS (swift) 開発
- Ruby on Rail でWebサービスを立ち上げる
- インターナショナルなチームでの開発
  - ただし、オフショア開発は自分が理想と考える開発はできないと考えてるので除く
- お仕事で Elm 使う
- お仕事で Haskell 使う
- JAM Stack
- Google Cloud Platform (GCP) に Function as a Service (FaaS) があるのできになる
- サーバレスアーキテクチャー
- 転職

## 職務経歴

### 2011/04/01 〜 : 株式会社タイムインタメーディア

職務: Webアプリケーションエンジニア

#### 2017/04 ~ 進行中 卒業生向けサービス開発
##### やったこと
- 開発リーダー
  - フレームワークに関するHow-to的なアドバイス
  - 実装の方針やレビュー
- 仕様の策定と設計
- バックエンドの技術選定・アーキテクチャ（一部)
- フロントエンドの技術選定・アーキテクチャ
  - Web APIの呼び出し前提なので、遅さを考えてアプリなのでSPAで体感速度をあげた
- 開発フローにPull Requestを導入した
- JSON Hyper Schemaを使ってフロントエンドとバックエンドの結合をちょっとだけ効率よくした
- API設計
- 画面のデザイン
##### 失敗
- フロントエンドにテスト書けてないこと
- バックエンドのテストが必要最低限のもしか書けてないこと
##### 触った技術
- Dynamics CRM 365
- .NET Framework 4.7
- ASP.NET Core 1.2
- JavaScript
- Vue.js 2.5
- React
- JSON Hyper Schema
- Azure
  - App Service
  - Azure Functions
  - Azure Service Bus

#### 2016/04 ~ 2017/04 教務システム・学生向けサービス開発
##### やったこと
- アーキテクチャ周りのヘルプ係
- 導入先システムに合わせたカスタマイズ部分の修正
- 学生向けサービスの開発
- 後輩のメンター
##### 失敗
- 入った時期が中途半端だったせいで、Azureのレガシーサービス(Cloud Services)が使われてしまった。もっと早く助言できれば回避できた可能性がある
- 入った時期が中途半端だったせいで、レガシーORM(Linq to SQL)が使われてしまった。もっと早く助言できれば回避できた可能性がある
##### 触った技術
- Linq to SQL
- ASP.NET Web Forms
- JavaScript
- jQuery 2.x
- Azure
  - Azure Cloud Services
  - Azure SQL Server
  - Azure Service Bus

#### 2014/04 ~ 現在    卒業生/寄付者管理システムの保守開発
##### やったこと
- DB設計
- 保守
- 新規開発
- 要件定義
- 散らかったドキュメントの整備
- リリースワークフローの改善
- マイクロリファクタリングによるコードの改善
##### 触った技術
- Dynamics CRM 2011
- .NET Framework 4.5.2/4.7
- ASP.NET MVC 5
- ASP.NET Web API 2
- ASP.NET Web Forms
- JavaScript
- React

#### 2014/04 ~ 2015/01 問い合わせ管理システム開発
##### やったこと
- オンプレミスのPHP製既存システムからAzureとASP.NETを使ったシステムに移行
- DB移行作業
- 要件定義
- 開発(DB+バックエンド+フロントエンド)
##### 触った技術
- .NET Framework 4.5.2
- ASP.NET MVC 5
- Entity Framework 5
- JavaScript
- Angular.JS 1.x
- Azure
  - Azure Cloud Services
  - Azure SQL Server

#### 2014/04 ~ 2016/04 学生向けポータルサイトと関連システムの保守開発
##### やったこと
- モバイル(Android/iOS)向けアプリの保守開発
- ポータルシステムとの連携サービスの保守
##### 触った技術
- .NET Framework 4.0
- SharePoint
- WCF Services
- HTML
- JavaScript
- jQuery 1 and 2系
- Android (KitKat)
- iOS 10

#### 2012/10 ~ 2014/03 治験業務の基幹システム保守開発
##### やったこと
- アジャイル開発
- DB〜フロントエンドまでの保守・開発
- 障害発生時の対応
- 要件定義
- フロントエンドをBootstrapを使ったモダンな画面にリニューアル
- バックエンドを最新のORMへ更新
##### 触った技術
- .NET Framework 4.0/4.5.2
- ASP.NET Web Forms
- Entity Framework 5
- JavaScript
- jQuery 1 and 2系

#### 2012/02 ~ 2012/09 ソーシャルネットワークサービス(SNS)開発
##### やったこと
- 全文検索の開発
##### 利用技術
- .NET Framework 4.5
- ASP.NET MVC 3
- Entity Framework 4
- Lucene.NET
- Azure
  - Azure Cloud Services
  - Blob Storage

#### 2011/04 ~ 2012/02 廃棄物管理表業務アプリの保守開発
##### やったこと
- REST APIによるデータ連携部分の保守開発
- フロント・サーバーの保守開発
##### 利用技術
- .NET Framework 3.5
- ASP.NET
- ClickOnce
